        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-md-12 grid-margin">
              <div class="row">
                <h3 class="font-weight-bold">Data Bimbingan</h3>
              </div>
            </div>
          </div>
          <div class="row">		
                    ...
                    <tr>
                      <td>
                        <a href="#" data-target="#editDataModal" class="edit" data-toggle="modal" 
                          ...
                          data-npm="{{ $d->npm }}"
                          data-judul="{{ $d->judul }}"
                          data-judul-english="{{ $d->judulEnglish }}">
                          <i class="ri-edit-2-line" data-toggle="tooltip" title="Edit"></i>
                        </a>
                        <a href="#" data-target="#deleteDataModal" class="delete" data-toggle="modal" data-id="{{ $d->id }}"><i class="ri-delete-bin-line" data-toggle="tooltip" title="Delete"></i></a>
                      </td>
                      <td>
                        @if (empty(auth()->user()->photo))
                      ...
                      <td>{{ $d->npm }}</td>
                      <td>{{ $d->judul }}</td>
                      <td>{{ $d->judulEnglish }}</td>
                      <td></td>
                      <td></td>
                      <td></td>
                      <td><a href="/dashboard/download/{{ $d->formpersetujuan }}">{{ $d->formpersetujuan }}</a></td>
                      <td></td>
                      <td></td>
                    </tr>
                  @endforeach
            ...
            <div id="addDataModal" class="modal fade">
              <div class="modal-dialog">
                <div class="modal-content">
                  <form action="/dashboard/bimbingan/databimbingan" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="modal-body">					
                      ...
                      <div class="form-group">
                        ...
                        <textarea name="judulEnglish" id="judulEng" class="form-control" rows="3" required></textarea>
                      </div>		
                      <div class="form-group">
                        <label>Form Persetujuan</label>
                        <input type="file" class="form-control-file" name="formpersetujuan" required>
                      </div>		
                    </div>
                ...
                <div class="modal-content">
                  <form id="editForm" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                      <div class="form-group">
                        <label>NPM</label>
                        <input type="text" name="npm" id="npmEdit" class="form-control">
                      </div>
                      <div class="form-group">
                        <input type="hidden" name="userId" value="{{ auth()->user()->id }}">
                      </div>	
                      <div class="form-group">
                        <label>Judul</label>
                        <textarea name="judul" id="judulEdit" class="form-control" rows="3"></textarea>
                      </div>
                      <div class="form-group">
                        <label>Judul English</label>
                        <textarea name="judulEnglish" id="judulEnglishEdit" class="form-control" rows="3"></textarea>
                      </div>	
                      <div class="form-group">
                        <label>Form Persetujuan</label>
                        <input type="file" class="form-control-file" name="formpersetujuan">
                      </div>							
                    </div>
                    <div class="modal-footer">
                      <input type="button" class="button-cancel button-modal" data-dismiss="modal" value="Cancel">
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function(){
      ...
      $('.edit').on('click', function (ev) {
        ...
        $pressedButton = $(this);
        var judulEnglish = $(this).data('judulEnglish'),
        $pressedButton = $(this);
        resourceId = $(this).data('id'),
        $pressedButton = $(this);

        $('#npmEdit').val(npm);
        $('textarea#judulEdit').val(judul);
        $('textarea#judulEnglishEdit').val(judulEnglish);
      });
      
      $('#editSubmitData').on('click', function (en) {
          en.preventDefault();
          var submitUrl = '/dashboard/bimbingan/databimbingan/' + resourceId,
          form = $('#editForm'); 
