        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-md-12 grid-margin">
              <div class="row">
                <h3 class="font-weight-bold">Persyaratan Awal</h3>
              </div>
            </div>
          </div>
          <div class="row">		
            ...
            <div class="table-responsive">
              <table class="table">
                ...
                <tbody>
                  @foreach ($persyaratanawal as $p)
                    <tr>
                      <td>
                        <a href="#" data-target="#editDataModal" class="edit" data-toggle="modal" 
                          data-id="{{ $p->id }}" 
                          data-totalsks="{{ $p->totalsks }}"
                          data-mk-nilai-d="{{ $p->mk_nilai_d }}"
                          data-ipk="{{ $p->ipk }}">
                      ...
                      <td>{{ $p->mk_nilai_d }}</td>
                      <td>{{ $p->ipk }}</td>
                      <td><a href="/dashboard/download/{{ $p->krs }}">{{ $p->krs }}</a></td>
                      <td><a href="/dashboard/download/{{ $p->transkrip }}">{{ $p->transkrip }}</a></td>
                      <td><a href="/dashboard/download/{{ $p->sertif1 }}">{{ $p->sertif1 }}</a></td>
                      <td><a href="/dashboard/download/{{ $p->sertif2 }}">{{ $p->sertif2 }}</a></td>
                      <td><a href="/dashboard/download/{{ $p->sertif3 }}">{{ $p->sertif3 }}</a></td>
                      <td></td>
                    </tr>
                  @endforeach
                </tbody>
            ...
            <div id="addDataModal" class="modal fade">
              <div class="modal-dialog">
                <div class="modal-content">
                  <form action="/dashboard/persyaratanawal" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="modal-body">			
                      ...
                      <div class="form-group">
                        ...
                        <input type="text" name="totalsks" class="form-control" required>
                      </div>
                      <div class="form-group">
                        <label>IPK</label>
                      ...
                      <div class="form-group">
                        <label>Total MK Nilai D</label>
                        <input type="text" name="mk_nilai_d" class="form-control" required>
                      </div>
                      <div class="form-group">
                        <label for="uploadKRS">KRS</label>
                        <input type="file" class="form-control-file" id="uploadKRS" name="uploadKRS" required>
                      </div>
                      <div class="form-group">
                        <label for="uploadTranskrip">Transkrip</label>
                        <input type="file" class="form-control-file" id="uploadTranskrip" name="uploadTranskrip" required>
                      </div>
                      <div class="form-group">
                        <label for="uploadSertif1">Sertifkat 1</label>
                        <input type="file" class="form-control-file" id="uploadSertif1" name="uploadSertif1" required>
                      </div>
                      <div class="form-group">
                        <label for="uploadSertif2">Sertifkat 2</label>
                        <input type="file" class="form-control-file" id="uploadSertif2" name="uploadSertif2" required>
                      </div>
                      <div class="form-group">
                        <label for="uploadSertif3">Sertifkat 3</label>
                        <input type="file" class="form-control-file" id="uploadSertif3" name="uploadSertif3" required>
                      </div>			
                    </div>
                ...
                <div class="modal-content">
                  <form id="editForm" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                      ...
                      <div class="form-group">
                        <label>Total SKS</label>
                        <input type="text" name="totalsks" id="totalsks" class="form-control" required>
                      </div>
                      <div class="form-group">
                        <label>Total MK Nilai D</label>
                        <input type="text" name="mk_nilai_d" id="mk_nilai_d" class="form-control" required>
                      </div>
                      <div class="form-group">
                        <label>IPK</label>
                      ...
                      <div class="form-group">
                        <label for="uploadKRS">KRS</label>
                        <input type="file" class="form-control-file" id="uploadKRS" name="krs">
                      </div>
                      <div class="form-group">
                        <label for="uploadTranskrip">Transkrip</label>
                        <input type="file" class="form-control-file" id="uploadTranskrip" name="transkrip">
                      </div>
                      <div class="form-group">
                        <label for="uploadSertif1">Sertifkat 1</label>
                        <input type="file" class="form-control-file" id="uploadSertif1" name="sertif1">
                      </div>
                      <div class="form-group">
                        <label for="uploadSertif2">Sertifkat 2</label>
                        <input type="file" class="form-control-file" id="uploadSertif2" name="sertif2">
                      </div>
                      <div class="form-group">
                        <label for="uploadSertif3">Sertifkat 3</label>
                        <input type="file" class="form-control-file" id="uploadSertif3" name="sertif3">
                      </div>			
                    </div>
                    <div class="modal-footer">
                      <input type="button" class="button-cancel button-modal" data-dismiss="modal" value="Cancel">
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function(){
      ...
      $('.edit').on('click', function (ev) {
        ...
        var totalsks = $(this).data('totalsks'),
        $pressedButton = $(this);
        var ipk = $(this).data('ipk'),
        $pressedButton = $(this);
        var mk_nilai_d = $(this).data('mkNilaiD'),
        $pressedButton = $(this);
        resourceId = $(this).data('id'),
        $pressedButton = $(this);

        $('#totalsks').val(totalsks);
        $('#mk_nilai_d').val(mk_nilai_d);
        $('#ipk').val(ipk);
      });
      
      $('#editSubmitData').on('click', function (en) {
          en.preventDefault();
          var submitUrl = '/dashboard/persyaratanawal/' + resourceId,
          form = $('#editForm'); 
