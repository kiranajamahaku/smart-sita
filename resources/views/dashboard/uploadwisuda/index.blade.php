        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-md-12 grid-margin">
              <div class="row">
                <h3 class="font-weight-bold">Upload Berkas Wisuda</h3>
              </div>
            </div>
          </div>
          <div class="row">		
            ...
            <div class="table-responsive">
              <table class="table">
                ...
                <tbody>
                    @foreach ($uploadwisuda as $u)
                    <tr>
                        <td>
                            <a href="#" data-target="#editDataModal" class="edit" data-toggle="modal" 
                            data-id="{{ $u->id }}" 
                            data-pengesahan="{{ $u->pengesahan }}"
                            data-foto="{{ $u->foto }}"
                            data-ijazah="{{ $u->ijazah }}"
                            data-bayarWisuda="{{ $u->bayarWisuda }}">
                            <i class="ri-edit-2-line" data-toggle="tooltip" title="Edit"></i>
                          </a>
                          <a href="#" data-target="#deleteDataModal" class="delete" data-toggle="modal" data-id="{{ $u->id }}"><i class="ri-delete-bin-line" data-toggle="tooltip" title="Delete"></i></a>
                        </td>
                        <td>
                          @if (empty(auth()->user()->photo))
                        ...
                        <td><a href="/dashboard/download/{{ $u->pengesahan }}">{{ $u->pengesahan }}</a></td>
                        <td><a href="/dashboard/download/{{ $u->foto }}">{{ $u->foto }}</a></td>
                        <td><a href="/dashboard/download/{{ $u->ijazah }}">{{ $u->ijazah }}</a></td>
                        <td><a href="/dashboard/download/{{ $u->bayarWisuda }}">{{ $u->bayarWisuda }}</a></td>
                    </tr>
                    @endforeach
                </tbody>
              </table>
            ...
            <div id="addDataModal" class="modal fade">
                <div class="modal-dialog">
                  <div class="modal-content">
                    <form action="/dashboard/uploadwisuda" method="POST" enctype="multipart/form-data">
                      @csrf
                      ...
                      <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label for="pengesahan">Pengesahan TA</label>
                            <input type="file" class="form-control-file" id="pengesahan" name="pengesahan" required>
                        </div>
                        ...
                        <div class="form-group">
                            <label for="bayarWisuda">Pembayaran Wisuda</label>
                            <input type="file" class="form-control-file" id="bayarWisuda" name="bayarWisuda" required>
                        </div>	
                  ...
                  <div class="modal-content">
                    <form id="editForm" method="POST" enctype="multipart/form-data">
                      ...
                      <div class="modal-body">
                        ...
                        <div class="form-group">
                          <label for="pengesahan">Pengesahan TA</label>
                          <input type="file" class="form-control-file" id="pengesahan" name="pengesahan">
                        </div>
                        <div class="form-group">
                          <label for="foto">Foto 3x4</label>
                        ...
                        <div class="form-group">
                          <label for="bayarWisuda">Pembayaran Wisuda</label>
                          <input type="file" class="form-control-file" id="bayarWisuda" name="bayarWisuda">
                        </div>		
                      </div>
                      <div class="modal-footer">
                        <input type="button" class="button-cancel button-modal" data-dismiss="modal" value="Cancel">
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function(){
      ...
      $('.edit').on('click', function (ev) {
        ...
        var pengesahan = $(this).data('pengesahan'),
        $pressedButton = $(this);
        var foto = $(this).data('foto'),
        $pressedButton = $(this);
        var ijazah = $(this).data('ijazah'),
        $pressedButton = $(this);
        var bayarWisuda = $(this).data('bayarWisuda'),
        $pressedButton = $(this);
        resourceId = $(this).data('id'),
        $pressedButton = $(this);

        $('#pengesahan').val(pengesahan);
        $('#ijazah').val(ijazah);
        $('#foto').val(foto);
        $('#bayarWisuda').val(bayarWisuda);
      });
      
      $('#editSubmitData').on('click', function (en) {
          en.preventDefault();
          var submitUrl = '/dashboard/uploadwisuda/' + resourceId,
          form = $('#editForm'); 
